...
class AnalogClock:
    ...
    def createCircleAndLabels(self):
        self.canvas.create_text(300, 20, text = "Evan Garcia's Nice Clock", font = ("times new roman", 20))
        ...
        for i in range(0,60):
            ...
            tipy = sin(angle) * 190
            if i % 5 == 0:
                self.canvas.create_text(300 + tipx, 300 - tipy, text = str(i), font = ("times new roman", 11))
                tipx = cos(angle) * 230
                tipy = sin(angle) * 230
                if j == 0:
                ...
                self.canvas.create_text(300 + tipx, 300 - tipy, text = "*",  font = ("times new roman", 11))
                tipx = cos(angle) * 210
                tipy = sin(angle) * 210
                self.canvas
    ...
    def TimeKeeper(self):  
        ...
        tipx = cos(angle) * lenMinuteAndSecHand
        tipy = sin(angle) * lenMinuteAndSecHand
        self.canvas.create_line(300, 300, 300 + tipx, 300 - tipy, width = 2, fill = "red", tags = "second")
